...
# ==================== DICTIONNAIRE DES MODULES PYTHON ====================
PYTHON_MODULES = {
    'requests': 'requests',
    'flask': 'Flask',
    'django': 'Django',
...
def attempt_install_pip(module_name, message):
    ...
    if package_name is None:
        ...
        return False
    try:
        bot.reply_to(message, f"Module `{module_name}` not found. Installing `{package_name}`...", parse_mode=None)
        command = [sys.executable, '-m', 'pip', 'install', package_name]
        logger.info(f"Running install: {' '.join(command)}")
        result = subprocess.run(command, capture_output=True, text=True, check=False, encoding='utf-8', errors='ignore')
        if result.returncode == 0:
            logger.info(f"Installed {package_name}. Output:\n{result.stdout}")
            bot.reply_to(message, f"Package `{package_name}` (for `{module_name}`) installed.", parse_mode=None)
            return True
        else:
...
def attempt_install_npm(module_name, user_folder, message):
    try:
        bot.reply_to(message, f"Node package `{module_name}` not found. Installing locally...", parse_mode=None)
        ...
        if result.returncode == 0:
            ...
            bot.reply_to(message, f"Node package `{module_name}` installed locally.", parse_mode=None)
            return True
        else:
...
# ==================== FONCTIONS D'EXÉCUTION DE SCRIPTS ====================
...
# ==================== FONCTIONS BASE DE DONNÉES (SUITE) ====================
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        bot.reply_to(message, f"Files extracted. Starting main script: `{main_script_name}`...", parse_mode=None)
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        try:
            owner_notification = (f"New user!\nName: {user_name}\nUser: @{user_username or 'N/A'}\n"
                                  f"ID: `{user_id}`\nBio: {user_bio}")
            bot.send_message(OWNER_ID, owner_notification, parse_mode=None)
            if photo_file_id:
    ...
    try:
        ...
        bot.send_message(chat_id, welcome_msg_text, reply_markup=main_reply_markup, parse_mode=None)
    except Exception as e:
...
def _logic_check_files(message):
    ...
    bot.reply_to(message, "Your files:\nClick to manage.", reply_markup=markup, parse_mode=None)
...
def _logic_uptime(message):
    ...
    bot.reply_to(message, f"Bot Uptime: `{uptime_str}`", parse_mode=None)
...
def _logic_run_all_scripts(message_or_call):
    ...
    reply_func(summary_msg, parse_mode=None)
...
def handle_file_upload_doc(message):
    ...
    try:
        try:
            ...
            bot.send_message(OWNER_ID, f"File '{file_name}' from {message.from_user.first_name} (`{user_id}`)", parse_mode=None)
        except Exception as e:
...
def handle_callbacks(call):
    ...
    try:
        if data == 'upload':
            ...
            uptime_str = get_uptime()
            bot.send_message(call.message.chat.id, f"Bot Uptime: `{uptime_str}`", parse_mode=None)
        else:
...
def check_files_callback(call):
    ...
    markup.add(types.InlineKeyboardButton("Back to Main", callback_data='back_to_main'))
    try:
        bot.edit_message_text("Your files:\nClick to manage.", chat_id, call.message.message_id, reply_markup=markup, parse_mode=None)
    except telebot.apihelper.ApiTelegramException as e:
    ...
    try:
        ...
        try:
            bot.edit_message_text(
                ...
                reply_markup=create_control_buttons(script_owner_id, file_name, is_now_running), parse_mode=None
            )
        except telebot.apihelper.ApiTelegramException as e:
            if "message is not modified" in str(e):
        ...
        if not is_bot_running(script_owner_id, file_name):
            ...
            try:
                bot.edit_message_text(
                    f"Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`\nStatus: Stopped",
                    chat_id_for_reply, call.message.message_id,
                    reply_markup=create_control_buttons(script_owner_id, file_name, False), parse_mode=None)
            except Exception as e:
    ...
    try:
        ...
        try:
            bot.edit_message_text(
                ...
                chat_id_for_reply, call.message.message_id, reply_markup=None, parse_mode=None
            )
        except Exception as e:
            logger.error(f"Error editing msg after delete: {e}")
            bot.send_message(chat_id_for_reply, f"Record `{file_name}` deleted.", parse_mode=None)
    except (ValueError, IndexError) as e:
...
def back_to_main_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(main_menu_text, chat_id, call.message.message_id,
                              reply_markup=create_main_menu_inline(user_id), parse_mode=None)
    except telebot.apihelper.ApiTelegramException as e:
        if "message is not modified" in str(e):
...
def process_broadcast_message(message):
    ...
    bot.reply_to(message, f"Confirm Broadcast:\n\n```\n{preview_text}\n```\n"
                          f"To **{target_count}** users. Sure?", reply_markup=markup, parse_mode=None)
...
def execute_broadcast(broadcast_text, photo_id, video_id, caption, admin_chat_id):
    ...
    for i, user_id_bc in enumerate(users_to_broadcast):
        try:
            if broadcast_text:
                bot.send_message(user_id_bc, broadcast_text, parse_mode=None)
            elif photo_id:
                bot.send_photo(user_id_bc, photo_id, caption=caption, parse_mode=None)
            elif video_id:
                bot.send_video(user_id_bc, video_id, caption=caption, parse_mode=None)
            sent_count += 1
        except telebot.apihelper.ApiTelegramException as e:
...
def list_admins_callback(call):
    ...
    try:
        ...
        bot.edit_message_text(f"Current Admins:\n\n{admin_list_str}", call.message.chat.id,
                              call.message.message_id, reply_markup=create_admin_panel(), parse_mode=None)
    except Exception as e:
